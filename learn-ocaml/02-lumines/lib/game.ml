...
let can_move t ~row ~col =
  ...
  if row < height && col < width && row >= 0 && col >= 0
    then (if ((Board.is_empty t {col = col; row = row}) && (Board.is_empty t {col = col; row = row + 1}) 
      && (Board.is_empty t {col = col + 1; row = row + 1}) && (Board.is_empty t {col = col + 1; row = row}))
      true
    else 
      false)
  else
...
let move_left t =
  ...
  if can_move t t.moving_piece_row (t.moving_piece_col - 1)
...
let tick t =
  ...
  if not (can_move t ~row:(t.moving_piece_row - 1) ~col:t.moving_piece_col)
    then if not (Board.add_piece t.board ~moving_piece:t.moving_piece ~col:t.moving_piece_col);
      then (
        t.game_over := true)
    else
      new_moving_piece t 
  else
    t.moving_piece_row <- t.moving_piece_row - 1; (* mutating the value *)
