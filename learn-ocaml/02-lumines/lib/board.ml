...
let test t ~(moving_piece:Moving_piece.t) ~col currbool i =
    if currbool
      then true
    else 
      (if is_empty t {col = col; row = i} then
        (if i > t.height && col > t.width then false  
            else
            (set t {col = col; row = i} (Some moving_piece.bottom_left); 
            set t {col = col; row = i + 1} (Some moving_piece.top_left);
            set t {col = col + 1; row = i + 1} (Some moving_piece.top_right);
            set t {col = col + 1; row = i} (Some moving_piece.bottom_right);
            true))
        else
          (false) )
      ;;
...
let add_piece t ~moving_piece ~col =
  (* TODO: insert the moving piece into the board, applying gravity
    ...
    let x = List.range 0 t.height in 
    List.fold ~init:false x ~f:(test t ~moving_piece ~col) 
;;
